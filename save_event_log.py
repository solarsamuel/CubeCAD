...
class MainWindow(QMainWindow):
    ...
    def save_event_log(self):
        ...
        filename = datetime.datetime.now().strftime("event_log_%Y-%m-%d_%H-%M-%S.txt")
        options = QFileDialog.Options()
        filepath, _ = QFileDialog.getSaveFileName(self, "Save Event Log", filename, "Text Files (*.txt);;All Files (*)", options=options)
        if filepath:
